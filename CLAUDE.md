...
| `/convert <app-name>` | Reads a Lovable vibe-coded app from `lovable-conversions/<app-name>/`, generates a plan directly from the source (no design/requirements phase), and scaffolds three repos in `repos/` |

**Standard pipeline**: create `ideas/<app-name>/ideas.md` → run `/design` → `/plan` → `/create`.

**Lovable pipeline**: drop Lovable export into `lovable-conversions/<app-name>/` → run `/convert`.
...
lovable-conversions/ ← YOU CREATE: lovable-conversions/<app-name>/ with Lovable export  (lovable pipeline)
